...
args_parser.add_argument('--model_dir', default="/users/fsimin/multilingual_pretrain/detoxify_models/checkpoint_en", type=str)
...
if __name__ == "__main__":
    ...
    model_en = AutoModelForSequenceClassification.from_pretrained(MODEL_PATH)
    tokenizer_en = AutoTokenizer.from_pretrained("FacebookAI/roberta-base")
    # for language, language_code in LANGS:
    pipeline = [
        JsonlReader(
            f"{args.data_dir}/english/",
            text_key="text",
            limit=25_000,
            shuffle_files=True,
        ),
        ...
        JsonlWriter(f"{args.output_dir}/en"),
    ]

    LocalPipelineExecutor(
        pipeline=pipeline,
        logging_dir=f"{args.log_dir}/en",
        tasks=4,
        # cpus_per_node=4,
        # time="01:00:00",
        # partition="normal",
